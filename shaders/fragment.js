...
float shade(vec2 uv, float t) {
    ...
    return pow((r.y + r.y) * max(.0, uv.y) + .1, 5.0);
}
...
vec3 color(float grad) {
 ...
 void mainImage( out vec4 fragColor, in vec2 fragCoord ){
    ...
    //  uv-=vec2(0.5);
    //  uv = scale(vec2(4))*uv;//SIZE
    //  uv+=vec2(0.5);
    vec2 translate = vec2(.7,.5);
    uv+=translate;
     float ff = 1.0 - uv.y;
     uv.x -= (iResolution.x / iResolution.y - 1.0) / 4.0;
     vec2 uv2 = uv;
     uv2.y = 1.0 - uv2.y;
     uv = polarMap(uv, 1.3, 1.0);
     uv2 = polarMap(uv2, 1.9, 1.0);
